...
export class GoogleAuthUseCase implements IGoogleAuthUseCase {
    ...
    async execute(code: string, role: string):
        ...
        const email = await this._googleAuthService.getGoogleEmail(code);

        let user: Gym | Client | Trainer | null = null;

        if (role === "gym") {
            user = await this._gymRepository.findByEmail(email);
        } else if (role === "client") {
            user = await this._clientRepository.findByEmail(email);
        } else if (role === "trainer") {
            user = await this._trainerRepository.findByEmail(email);
        }
        if (!user) {
            if (role === "gym") {
                const randomPassword = Math.random().toString(36).slice(-8);
                const newGym = new Gym("", email, randomPassword, role);
                user = await this._gymRepository.create(newGym);
            } else {
